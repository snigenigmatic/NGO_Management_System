...
-- View: NGO summary - total donations and donor count (for KPI dashboard)
...
-- View: Monthly donations (for analytics page)
CREATE OR REPLACE VIEW vw_monthly_donations AS
SELECT 
    DATE_FORMAT(d.Donation_date, '%Y-%m') AS `year_month`,
    d.NGO_ID AS `NGO_ID`,
    SUM(d.Amount) AS `total_amount`,
    COUNT(*) AS `donation_count`
FROM Donation d
GROUP BY d.NGO_ID, DATE_FORMAT(d.Donation_date, '%Y-%m');

-- View: Event volunteer hours (for analytics page)
...
GROUP BY e.Event_ID, e.Event_Type, e.Location;

-- View: Active vendor contracts (for analytics page)
