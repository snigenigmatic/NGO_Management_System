...
DELIMITER $$
CREATE FUNCTION get_ngos_total_donations(p_ngo_id INT) RETURNS DECIMAL(12, 2) DETERMINISTIC
BEGIN
...
BEGIN
    ...
    WHERE NGO_ID = p_ngo_id;
    IF v_count = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'NGO does not exist';
    END IF;
    
    IF p_amount <= 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Donation amount must be positive';
    END IF;
...
DELIMITER $$
CREATE TRIGGER trg_before_insert_donation
...
BEGIN
    IF (OLD.Amount IS NULL AND NEW.Amount IS NOT NULL) OR (OLD.Amount <> NEW.Amount) THEN
...
DELIMITER $$
CREATE TRIGGER trg_after_insert_event
AFTER INSERT ON Event FOR EACH ROW
BEGIN
    IF NEW.Venue_ID IS NOT NULL THEN
        UPDATE Venue SET Status = 'Booked' WHERE Venue_ID = NEW.Venue_ID;
    END IF;
END $$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE create_event_and_book_venue(
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Failed to create event and book venue (rolled back)';
    END;
    ...
    IF p_venue_id IS NOT NULL THEN
        IF (SELECT COUNT(*) FROM Venue WHERE Venue_ID = p_venue_id AND Status = 'Available') = 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Venue not available or does not exist';
        END IF;
    END IF;
    ...
    IF p_vendors IS NOT NULL AND JSON_LENGTH(p_vendors) > 0 THEN
        SET n = JSON_LENGTH(p_vendors);
        SET i = 0;
        WHILE i < n DO
            SET vendor_obj = JSON_EXTRACT(p_vendors, CONCAT('$[', i, ']'));
            SET vendor_id = CAST(JSON_EXTRACT(vendor_obj, '$.vendor_id') AS SIGNED);
            SET v_cost = CAST(JSON_EXTRACT(vendor_obj, '$.cost') AS DECIMAL(10, 2));
            SET v_details = JSON_UNQUOTE(JSON_EXTRACT(vendor_obj, '$.details'));
            
            INSERT INTO Event_Vendor (Event_ID, Vendor_ID, Service_Details, Cost, Contract_Date)
            VALUES (new_event_id, vendor_id, v_details, v_cost, CURDATE());
    ...
    IF p_venue_id IS NOT NULL THEN
        UPDATE Venue SET Status = 'Booked' WHERE Venue_ID = p_venue_id;
    END IF;
...
DELIMITER $$
CREATE PROCEDURE show_ngo_totals()
BEGIN
    SELECT NGO_ID, NGO_Name, get_ngos_total_donations(NGO_ID) AS total_donations FROM NGO;
END $$
DELIMITER ;